...
> - **🐞→✅ Black-box confirmation FIXED (`eb7dd55`, building).** User: the "X uninstalled" popup was an empty black box at the toast position → system `Toast` renders black on this ROM (targetSDK 28). Replaced the 3 Steam uninstall Toasts with **`UninstallResultBar`** (themed auto-dismiss ~2.2 s snackbar-style bar, non-interactive). **CI build `28717094726` RUNNING** on `eb7dd55` → deliver APK on green.
...
> **NEXT:** build `28717094726` green → deliver black-box-fix APK → user confirms the confirmation bar shows real text → then merge decision. Detail below + memory `project_bannerlator_download_manager`.
