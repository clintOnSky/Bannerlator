...
> **NEXT merge-prep (1 left):** #3 fix cosmetic `Depot N complete: X KB` under-report (the per-depot byte summary in the shared debug log undercounts; download itself is correct) → then MERGE `feat/steam-goldberg-patcher` to main → THEN cross-store Download Manager. At stable release hand out credits (upstream OSS + our own work → GitHub release notes + repo credits).
