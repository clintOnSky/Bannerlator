...
> **⏸️ CURRENT STATE (2026-07-04) — bug found in testing + building uninstall feedback.** Branch `feat/download-manager` (off main, NOT merged). DL Manager v1 DEVICE-PROVEN; landing-screen toggle works. **Two open threads from the latest test:**
> 1. **🐞 Uninstall→reinstall-same-session bug (diagnosed, repro requested).** User uninstalled HL2 then immediately reinstalled: detail page stuck `0% (0 KB / 8.4 GB)`, DL-manager card wrongly showed `100% (8.4/8.4)`, and Steam Library stuck "Fetching app records (23/372)". **Log+DB diagnosis (evidence in `~/scratchpad/steam.db`, `hl2full.txt`):** uninstall was CLEAN (`is_installed=0`, dir gone); `steam_downloads` row = HL2 `status='queued', 0/8.99 GB` → **download never left the queue** (no HL2 dir ever created). No crash/OOM — the stuck process (pid 15684) was externally force-stopped at 14:30. **Two bugs:** (a) DL-manager "100%" = **stale in-memory byte counters** on the reused `DownloadEntry` (DB proves 0 bytes) — detail page reads live truth; (b) real bug = **CM/netThread wedge** — the queued download + the library PICS fetch both stall on the shared Steam CM after a same-session uninstall→reinstall. Release APK logs nothing (debug toggle off) so cause unconfirmed. **Awaiting user repro WITH "Log debug session" ticked** (+ control: restart-app-then-reinstall) to capture engine logs. See memory `project_bannerlator_download_manager`.
> 2. **🔨 IN PROGRESS — uninstall feedback (user-requested).** Old uninstall was fire-and-forget (`Thread{ deleteRecursively() }.start()`, UI flips to "done" while a multi-GB delete still runs, no confirmation, no verify) — and that async delete is a plausible contributor to the reinstall race. **Just implemented (uncommitted):** new `StoreUninstaller.kt` (`run(installDir, mark, onResult)` = mark+delete off UI thread, **verify dir actually gone**, callback on main thread) + `UninstallProgressDialog` (blocking spinner "Uninstalling <game>…"). Wired into all 3 uninstall sites (`SteamGamesActivity`, `SteamGameDetailActivity`, `DownloadManagerActivity`) → spinner during delete, then success/failure **Toast**. **NOT yet building/committed.** Deferred (#3): the actual reinstall-guard (block re-queue until delete completes) — hold until repro confirms the race.
>
> Steam/Goldberg already merged to main (`c89dc03`, no release, vc37/2.2.2). Last delivered APK still `bannerlator-dlmgr-twobar-landing-52e7e38-standard.apk` (`e0b173cd9f01`). PARKED: Epic/GOG/Amazon registry wiring. NEXT: commit+build uninstall-feedback → deliver → user repro of the reinstall bug → then merge decision. Detail in entries below + memory `project_bannerlator_download_manager`.
...
> - **NEXT:** commit (The412Banner) → push `feat/download-manager` → dispatch combined CI build → deliver standard APK → user tests the toast/spinner + runs the reinstall repro.
