...
> **→ NEW ACTIVE BRANCH `feat/download-manager-stores` (off main `ef717fb`):** route EpicDownloadManager/GogDownloadManager/AmazonDownloadManager into the SAME `DownloadRegistry` as live producers (mirroring the Steam `SteamDepotDownloader`+`SteamLibrarySync` pattern). android-app-engineer recon in flight to map each engine's progress contract → phased wiring plan. Parked: DepotSizeResolver true-size spec (memory).
