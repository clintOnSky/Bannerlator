...
> **⏸️ CURRENT STATE (2026-07-04) — 🎉 Download Manager v1 MERGED TO MAIN (`ef717fb`, fast-forward); now wiring Epic/GOG/Amazon into it on a new branch.** Everything device-proven: DL Manager v1, two-bar, verified uninstall, wedge fix, black-box fix, **Bug-1 size-text fix (HL2 full download, 10 GB on disk, no overshoot)**, landing-screen toggle. Still vc37/2.2.2, NO release cut. `feat/download-manager` kept.
> **→ NEW ACTIVE BRANCH `feat/download-manager-stores` (off main `ef717fb`):** route Amazon/GOG/Epic into the SAME `DownloadRegistry` as live producers, wired at the Activity callback seam (engines untouched). Order Amazon→GOG→Epic (lowest risk first). Recon plan done + saved to memory. **✅ Phase A (Amazon) BUILT + DELIVERED — commit `bc6d27c`, CI `28719688616` GREEN:** new `StoreDownloadHooks.kt` shim + `AmazonLibrarySync.kt` seeder + wired BOTH Amazon install entry points (detail + games-list) with real byte progress + `DownloadRegistry.init()`/seed in onCreate. **Standard APK DELIVERED** `/sdcard/Download/bannerlator-amazon-dlmgr-bc6d27c-standard.apk` (589,563,204 B, sha256 `d0ad8a2e094dcb94…c6448`, verified vs CI, media-scanned); NOT yet installed — **⏳ user device-testing now.** **Device-test checklist:** (1) fresh Amazon install shows a LIVE download row + moving byte bar on BOTH entry points (detail + games-list); (2) Cancel clears the row; (3) INSTALLED game survives an app kill (Library seed); (4) Uninstall clears the Library row. Then Phase B GOG. Parked: DepotSizeResolver true-size spec (memory).
