...
> **NEXT merge-prep (2 left):** #2 reconcile main's 4 commits into the branch; #3 fix cosmetic `Depot N complete: X KB` under-report → then MERGE `feat/steam-goldberg-patcher` to main → THEN cross-store Download Manager. At stable release hand out credits (upstream OSS + our own work → GitHub release notes + repo credits).
